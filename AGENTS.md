...
It renders `.chezmoi.toml.tmpl` with `chezmoi init` — including, beyond the default answers, `secrets.profile`'s "work"/"custom"/"none" branches and the less-common `shell.ssh.agent`/`kitty.quake_key`/`git.format` values, since only `chezmoi init` itself ever evaluates that file — dry-runs the whole tree against every `.github/fixtures/ci-data-*.yaml` fixture (every optional path enabled, plus each mutually-exclusive toggle variant), then does a REAL apply into a scratch destination, checks it round-trips (`chezmoi diff` is empty after a second apply) and passes `chezmoi verify`. It lints the *rendered* shell (shellcheck for bash, `zsh -n` for zsh, `fish --no-execute` for fish, `nu -c "nu-check --debug ..."` for nu) and the *rendered* YAML/JSON/JSONC/TOML config templates, parses every rendered Windows script (not just the `.chezmoiscripts/` hooks), runs PSScriptAnalyzer (including a PowerShell-5.1-compatibility gate on `bootstrap.ps1`), `test-repo-rules.py` (hook guards, `modify_` shape, `private_` secret files, `run_onchange_` hash comments), markdownlint and actionlint, checks Markdown links, exercises the secret templates against a stub `bw`, and re-verifies the pinned checksums. It uses a scratch config, cache, persistent state and destination, so it never touches your real `$HOME` (not `~/.cache/chezmoi` or `~/.config/chezmoi` either). A check prints `SKIP` when its tool is not installed.
...
| `home/dot_local/share/nredf/shell/nushell/` | The Nushell equivalent — `env.nu.tmpl`/`config.nu.tmpl`, **not** under `dot_local/share` for its actual entry points. Nu's config directory is platform-specific and resolved before nu reads any of its own files, so thin stubs (`home/dot_config/nushell/`, `home/private_Library/private_Application Support/nushell/`, `home/AppData/Roaming/nushell/`) just `source` this canonical tree — see `docs/shells.md`. **`home/dot_config/nushell/` deploys on every OS, not just Linux**: nu prefers `$XDG_CONFIG_HOME/nushell` over its OS-native path the instant `XDG_CONFIG_HOME` is set, which every other shell here already does — so both stubs have to exist on macOS/Windows too, or nu silently bootstraps its own vanilla config instead of reading ours. |
| `home/dot_local/share/nredf/shell/pwsh/` | The PowerShell equivalent — `Profile.ps1`, `Sources.ps1`, `Defaults.ps1`, `PSReadLine.ps1`, `Functions.bundle.ps1.tmpl`, and `Functions/` (capitalized, unlike the other shells' `functions/`). Thin stubs in `home/Documents/PowerShell/` (`Microsoft.PowerShell_profile.ps1`) dot-source this canonical tree. |
| `docs/` | User documentation. |

**Function bundles are generated.** `functions.bundle.tmpl` (bash/zsh/fish/nu) globs its `functions/` directory and `Functions.bundle.ps1.tmpl` (PowerShell) globs its `Functions/` directory, so a new function file is picked up automatically. Never re-introduce a hand-maintained include list: the loaders only fall back to the directory when the bundle is *absent*, so anything missing from a list is silently never loaded. **Nu has no such fallback at all** — its `source` requires the target to already exist on disk before the file is even parsed, so the generated bundle is nu's only load path, not a fast path.
...
- **`.chezmoiignore` matches target paths.** Gate per-OS files there (for example `.config/systemd` and `.config/Code` are Linux-only — `.config/nushell` is *not*, see the `nushell/` row in §2's table), not in the file itself.
...
- **Never prompt, block or print on startup.** A non-interactive login shell (`bash -lc`, `ssh host cmd`) must produce no stdout. Diagnostics go to stderr. Anything that reads `/dev/tty` needs a timeout, and "unanswered" must not be recorded as "no". **Fish reads `config.fish` for every invocation, interactive or not** (unlike bash's `[ -z "$PS1" ] && return`), so `fish/rc.tmpl` gates everything on `status is-interactive` itself rather than relying on a separate non-interactive entry file. **Nu skips loading `env.nu`/`config.nu` for a plain non-interactive `nu -c`**, but a login-shell invocation (`nu --login -c ...`, some `ssh host cmd` setups) still loads them non-interactively. `config.nu.tmpl` deliberately does *not* also gate everything on `$nu.is-interactive` as defense in depth — that was tried and reverted after it made every tool integration (atuin, carapace, oh-my-posh, mise, zoxide, fzf) silently no-op, since a `--login -c` invocation reports `$nu.is-interactive` as false too. Instead, only the two operations that actually must never run non-interactively (the aqua GitHub-token wizard, zellij auto-attach) carry their own direct guards. Neither fish's `read` nor nu's `input` has a timeout flag at all (unlike bash's `read -t`); the token-setup wizard shells out to `timeout`/`gtimeout` if one is installed and otherwise falls back to an untimed prompt, still gated on a real interactive tty.

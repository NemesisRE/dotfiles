...
| Same-session cached tool-init refresh (atuin/zoxide/mise/oh-my-posh/carapace/fzf) | Nushell | nu's `source` requires the target file to exist on disk *before the file that sources it is even parsed* — true even inside a runtime `if path exists` guard. A cache regenerated this session can only be `source`d starting the *next* shell start. | Chezmoi seeds an empty placeholder (`create_`/`empty_`) for each cache file so a cold `source` never fails outright; a stale-but-present cache is used for the rest of the current session and refreshes for the next one. |
